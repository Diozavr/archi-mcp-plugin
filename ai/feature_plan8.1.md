...
1) Базовая конфигурация Maven
   ...
   - Статус: [done]
   - Сделано: свойства и каталоги настроены в `pom.xml`.
   - Осталось: —
2) Зависимости тестов
   - Добавить `junit:junit:4.13.2:test`, `org.hamcrest:hamcrest:2.2:test`.
   - Статус: [done]
   - Сделано: зависимости добавлены в `pom.xml`.
   - Осталось: —
3) Плагины сборки
   ...
   - Статус: [done]
   - Сделано: плагины настроены в `pom.xml`, `MANIFEST.MF` используется из `META-INF`.
   - Осталось: —
4) Исключение интеграционных тестов (если есть)
   ...
   - Статус: [done]
   - Сделано: тесты с суффиксом `IT` исключены в `surefire`.
   - Осталось: —
5) Копирование артефактов плагина
   ...
   - Статус: [done]
   - Сделано: ресурсы и манифест подключены через `resources` и `jar` плагины.
   - Осталось: —
6) Проверка локально
   - `mvn -q -DskipTests package` — быстрый прогон упаковки.
   - `mvn -q test` — запуск юнит‑тестов.
   - Статус: [blocked]
   - Сделано: добавлен скрипт `fetch-archi-deps.sh` для загрузки библиотек Archi, прокси настроен.
   - Осталось: убедиться, что скрипт подтягивает зависимости и Maven успешно соберёт проект
